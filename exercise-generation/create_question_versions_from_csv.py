...
class MoodleQuestion:
    def __init__(self, topic_number, topic_id, exercise_number, exercise_part, topic_label, exercise_description, exercise_variables, exercise_text, exercise_content, exercise_hint, custom_general_feedback, exercise_note, custom_prt, add_prt_node_on_not_correct, add_prt_node_wa, custom_input, custom_seed, question_template_file_path="default_question.xml"):
        ...
        # print(f"{self.topic_number} {self.topic_id} {self.exercise_number} {self.exercise_part}")
...
#%%-----------------------------------UPDATE----------------------------------
#df = pd.read_csv("exercises_eak.csv", delimiter=";")
#df = pd.read_csv("exercises_mixed.csv", delimiter=";")
#df = pd.read_csv("exercises_structure_2.csv", delimiter=";")
#df = pd.read_csv("exercises_structure_3_update_after_first_surveys.csv", delimiter=";")
...
def pa_version_add_to_script_text(**kwargs):
    ...
    if Question.exercise_hint != "" and not pd.isna(Question.exercise_hint):
        ...
        if(lang != None and lang != "de"):
            hint_text = "Good luck with solving this task!"
...
#%%-----------------------------------ENGLISH VERSIONS--------------------
print("english ver")
df = pd.read_csv("exercises_en.csv", delimiter=";")
...
Exercises_To_Parse = []
for data in to_parse.itertuples():
    Exercise_To_Parse = MoodleQuestion(data.topic_number, data.topic_id, data.exercise_number, data.exercise_part, data.topic_label, data.exercise_description, data.exercise_variables, data.exercise_text, data.exercise_content, data.exercise_hint, data.custom_general_feedback, data.exercise_note, data.custom_prt, data.add_prt_node_on_not_correct, [data.add_prt_node_wa1, data.add_prt_node_wa2, data.add_prt_node_wa3], data.custom_input, data.custom_seed, question_template_file_path="default_question_en.xml")
    Exercises_To_Parse.append(Exercise_To_Parse)


#%%------------------------------------TEST----------------------------------

TestVersion_en = QuestionPool(QuestionVersion("test_en"), Exercises_To_Parse)
TestVersion_en.write_to_file("output_en.xml")

#%%------------------------------------PREVIEW---------------------------------
NormalVersion_en = QuestionPool(QuestionVersion("normal_en", False, '<script src="https://marvin.hs-bochum.de/~mneugebauer/alquiz-qpool-normal.js"></script>', "start-normal.xml", "config-normal.xml", callback_last_quiz_element_text=normal_version_config_text, Exercises=Exercises_To_Parse), Exercises_To_Parse)
NormalVersion_en.write_to_file()

#%%------------------------------------ROLE-PLAY-GAME----------------------------------
RPGVersion_en = QuestionPool(QuestionVersion("rpg_en", True, script_on_init='<script>document.addEventListener("DOMContentLoaded", function() { window.location.href = document.querySelector("[id*=quiznavbutton]").href; document.querySelectorAll(".back_instruction").forEach(function(elem) { elem.style.display = "block"; }); });</script>\n<p class="back_instruction" style="display:none;">To return to the game, please click on the first element in the quiz navigation.</p>\n', custom_first_elements_filepath="start-rpg.xml", fixed_seed_amount=rpg_fixed_seed_amount, show_hint=True, callback_first_quiz_element_text=rpg_version_insert_config_text, callback_change_before_clone=rpg_version_normalize_input_to_algebraic, Exercises=Exercises_To_Parse, seed_limit=rpg_fixed_seed_amount, lang="en"), Exercises_To_Parse)
RPGVersion_en.write_to_file()

#%%------------------------------------INTELLIGENT TUTORING SYSTEM----------------------------------
InstantTutoringVersion_en = QuestionPool(QuestionVersion("instant-tutoring_en", True, '<script src="https://marvin.hs-bochum.de/~mneugebauer/alquiz-qpool-instant-tutoring-en.js"></script>', "start-instant-tutoring_en.xml", "config-instant-tutoring.xml", it_fixed_seed_amount, True, callback_last_quiz_element_text=it_version_config_text, callback_add_to_script=it_version_add_to_script_text, callback_change_before_clone=it_version_change_text, Exercises=Exercises_To_Parse, seed_limit=it_fixed_seed_amount, lang="en"), Exercises_To_Parse)
InstantTutoringVersion_en.write_to_file()

#%%------------------------------------PA AND ENDBOSS-----------------------
PAEndbossVersion_en = QuestionPool(QuestionVersion("pa_en", False, '<script src="https://marvin.hs-bochum.de/~mneugebauer/alquiz-qpool-pa-en.js"></script>', "start-instant-tutoring_en.xml", show_hint=False, custom_last_elements_filepath="config-instant-tutoring.xml", callback_add_to_script=pa_version_add_to_script_text, callback_last_quiz_element_text=it_version_config_text, Exercises=Exercises_To_Parse), Exercises_To_Parse)
PAEndbossVersion_en.write_to_file()
